...
**Implication for new-item defaults:** when a new claim/qualifier needs to appear on item creation for a specific table, the correct place to express that is a `====<table>====` entry in `Ui_SortedProperties_NewItem` (presence and order) — not a hardcoded `if (props.table === 'x') { ... }` branch in `frontend/components/item/Create.vue`. Hardcoding per-table property IDs there bypasses the wiki-driven config, is easy to get wrong during merges (see the P799/P106 gating regression across PRs #475/#476/#478, where per-table `if` guards were dropped during a merge and silently became unconditional for every table), and requires a code deploy instead of a wiki edit for what should be config-only changes.

**Implication for item-creation *semantics*** (which claims are required, the auto-generated label, default claim values, save-time claim injection, aliases): that lives in `frontend/service/item-forms/<table>.js`, one pure-data module per table, driven by a shared engine (`frontend/service/item-forms/engine.js`) — see [`docs/frontend/item-forms.md`](docs/frontend/item-forms.md). `required` is *derived* from the label recipe, never configured separately, so the two can't drift out of sync the way they used to (#527). As with the wiki config above: never add a `props.table === 'x'` conditional to a component for this — extend a table's item-forms module, or the engine's `Source`/module contract if the table needs a genuinely new mechanism.
