        ...
        input: {
          ...
          author: {
            ...
            autocomplete: {
              query:
              `
              SELECT DISTINCT ?target_item ?analytic_item ?label ?desc ?lang ?db ?bg {
                {
                  {
                    SELECT DISTINCT ?target_item ?db ?bg {
                      ?item wdt:P476 ?pbid .
                      FILTER regex(?pbid, '{{database}} {{table}} ') .
                      BIND(STRBEFORE(?pbid, ' ') AS ?db) .
                      {{bitagapGroupFilter}}
                ...
                {
                  {  
                    SELECT DISTINCT ?analytic_item ?db ?bg {
          ...
          associated_person: {
            ...
            autocomplete: {
              ...
              SELECT DISTINCT ?target_item ?label ?desc ?lang ?db ?bg {
                {
                  SELECT ?target_item ?label ?db ?bg {
                    {
                      ?item wdt:P476 ?pbid .
                      FILTER regex(?pbid, '{{database}} {{table}} ') .
                      BIND(STRBEFORE(?pbid, ' ') AS ?db) .
          ...
          place_composition: {
            ...
            autocomplete: {
              query:
              `
              SELECT DISTINCT ?target_item ?label ?lang ?db ?bg {
                {
