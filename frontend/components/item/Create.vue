      ...
      <template v-if="isUserLogged">
        ...
        <v-row class="mt-2" dense>
          ...
          <v-tooltip bottom>
            ...
            <span v-if="isCreateDisabled" class="text-no-wrap">{{ $t('messages.error.inputs.fill') }}</span>
  ...
  computed: {
    ...
    isCreateDisabled () {
      return (
        ...
        Object.values(this.claims).some(claimArray => claimArray.some(entry =>
          entry?.value == null || entry?.value === '' ||
          Object.entries(entry.qualifiers || {}).some(([key, value]) => key == null || key === '' || value == null || value === ''))) ||
        Object.values(this.initialClaims).some(claim =>
          ...
          Object.values(claim.qualifiers || {}).some(q => !q?.datavalue?.value || Object.values(q?.datavalue?.value).some(val => val == null || val === '')))
      )
    },
    pbid () {
